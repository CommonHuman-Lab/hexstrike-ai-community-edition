...
class OutputParser:
    ...
    def parse_sqlmap_injection(output: str) -> Optional[Dict[str, str]]:
        ...
        return {"vulnerable": True, "type": injection_type, "parameter": parameter}
...
class AssertionHelpers:
    ...
    def assert_has_ports(output: str, expected_ports: List[int]):
        ...
        assert not missing_ports, f"Missing expected ports: {missing_ports}. Found: {found_ports}"
    ...
    def assert_has_service(output: str, port: int, expected_service: str):
        ...
        assert port in services, f"Port {port} not found in output. Available ports: {list(services.keys())}"

        assert (
            services[port] == expected_service
    ...
    def assert_has_vulnerability(output: str, vuln_id: str):
        ...
        assert vuln_id in output, f"Vulnerability {vuln_id} not found in output"
    ...
    def assert_command_success(result: Dict[str, Any]):
        ...
        assert result.get("success", False), f"Command failed: {result.get('error', 'Unknown error')}"

        assert result.get("returncode", -1) == 0, f"Command returned non-zero exit code: {result.get('returncode')}"
...
class TestDataGenerator:
    ...
    def generate_target_url(
        host: str = "example.com", scheme: str = "http", port: Optional[int] = None, path: str = ""
    ) -> str:
    ...
    @staticmethod
    def generate_port_scan_result(open_ports: List[int], services: Optional[Dict[int, str]] = None) -> Dict:
        """Generate a port scan result"""
        if services is None:
            services = {22: "ssh", 80: "http", 443: "https", 3306: "mysql", 8080: "http-proxy"}
    ...
    def generate_vulnerability(
        severity: str = "medium", name: str = "Test Vulnerability", description: str = "Test description"
    ) -> Dict:
        """Generate a vulnerability entry"""
        cvss_scores = {"critical": 9.0, "high": 7.5, "medium": 5.0, "low": 3.0, "info": 0.0}
    ...
    @staticmethod
    def generate_scan_report(tool: str, target: str, findings: List[Dict]) -> Dict:
    ...
    def generate_command_result(
        command: str, success: bool = True, stdout: str = "", stderr: str = "", duration: float = 1.0
    ) -> Dict:
...
class ComparisonHelpers:
    ...
    def assert_dict_contains(actual: Dict, expected: Dict, path: str = ""):
        ...
        for key, value in expected.items():
            ...
            if isinstance(value, dict):
                ComparisonHelpers.assert_dict_contains(actual[key], value, current_path)
            else:
