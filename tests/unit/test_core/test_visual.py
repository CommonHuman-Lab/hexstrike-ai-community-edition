...
class TestProgressBars:
    ...
    def test_render_progress_bar_different_styles(self):
        ...
        for style in styles:
            progress = ModernVisualEngine.render_progress_bar(0.75, width=40, style=style, label=f"Test {style}")
            assert isinstance(progress, str)
            assert "75.0%" in progress

    def test_render_progress_bar_with_eta(self):
        """Test progress bar with ETA information"""
        progress = ModernVisualEngine.render_progress_bar(0.5, width=40, label="Test", eta=30.5)
        assert isinstance(progress, str)
    ...
    def test_render_progress_bar_with_speed(self):
        ...
        progress = ModernVisualEngine.render_progress_bar(0.5, width=40, label="Test", speed="100 KB/s")
        assert isinstance(progress, str)
...
class TestDashboard:
    ...
    def test_create_live_dashboard_single_process(self):
        ...
        processes = {12345: {"status": "running", "command": "nmap -sV target.com", "duration": 30.5}}
    ...
    def test_create_live_dashboard_truncates_long_commands(self):
        ...
        processes = {12345: {"status": "running", "command": "a" * 100, "duration": 10}}  # Very long command
        dashboard = ModernVisualEngine.create_live_dashboard(processes)
        assert isinstance(dashboard, str)
...
class TestVulnerabilityCards:
    ...
    def test_format_vulnerability_card_basic(self):
        ...
        vuln_data = {"severity": "high", "name": "SQL Injection", "description": "SQL injection in login form"}
    ...
    def test_format_vulnerability_card_has_colors(self):
        ...
        vuln_data = {"severity": "critical", "name": "Critical Bug", "description": "Very dangerous"}
        card = ModernVisualEngine.format_vulnerability_card(vuln_data)
...
class TestErrorCards:
    ...
    def test_format_error_card_basic(self):
        ...
        card = ModernVisualEngine.format_error_card("ERROR", "nmap", "Connection timeout")
        assert isinstance(card, str)
        assert "nmap" in card
    ...
    def test_format_error_card_different_types(self):
        ...
        for error_type in error_types:
            card = ModernVisualEngine.format_error_card(error_type, "test_tool", "Test error message")
            assert isinstance(card, str)
...
class TestToolStatus:
    ...
    def test_format_tool_status_running(self):
        ...
        status = ModernVisualEngine.format_tool_status("nmap", "RUNNING", "target.com", progress=0.5)
    ...
    def test_format_tool_status_success(self):
        ...
        status = ModernVisualEngine.format_tool_status("gobuster", "SUCCESS", "http://example.com")
        assert isinstance(status, str)
        assert "SUCCESS" in status.upper()

    def test_format_tool_status_failed(self):
        """Test tool status for failed execution"""
        status = ModernVisualEngine.format_tool_status("sqlmap", "FAILED", "http://example.com")
        assert isinstance(status, str)
    ...
    def test_format_tool_status_with_progress(self):
        ...
        status = ModernVisualEngine.format_tool_status("nuclei", "RUNNING", "target.com", progress=0.75)
        assert isinstance(status, str)
        assert "75.0%" in status

    def test_format_tool_status_without_progress(self):
        """Test tool status without progress bar"""
        status = ModernVisualEngine.format_tool_status("nikto", "RUNNING", "target.com", progress=0.0)
        assert isinstance(status, str)
...
class TestSectionHeaders:
    ...
    def test_create_section_header_with_color(self):
        ...
        header = ModernVisualEngine.create_section_header("Results", icon="📊", color="HACKER_RED")
        assert isinstance(header, str)
...
class TestCommandFormatting:
    ...
    def test_format_command_execution_starting(self):
        ...
        formatted = ModernVisualEngine.format_command_execution("nmap -sV target.com", "STARTING")
        assert isinstance(formatted, str)
    ...
    def test_format_command_execution_running(self):
        ...
        formatted = ModernVisualEngine.format_command_execution("gobuster dir -u http://target.com", "RUNNING")
        assert isinstance(formatted, str)
    ...
    def test_format_command_execution_failed(self):
        ...
        formatted = ModernVisualEngine.format_command_execution("nuclei -u target.com", "FAILED", duration=10.0)
        assert isinstance(formatted, str)
        assert "FAILED" in formatted.upper()

    def test_format_command_execution_timeout(self):
        """Test formatting for timed out command"""
        formatted = ModernVisualEngine.format_command_execution("nikto -h target.com", "TIMEOUT", duration=300.0)
        assert isinstance(formatted, str)
    ...
    def test_format_command_execution_truncates_long_commands(self):
        ...
        long_command = "a" * 100
        formatted = ModernVisualEngine.format_command_execution(long_command, "RUNNING")
        assert isinstance(formatted, str)
        assert "..." in formatted
    ...
    def test_format_command_execution_without_duration(self):
        ...
        formatted = ModernVisualEngine.format_command_execution("test command", "RUNNING", duration=0.0)
        assert isinstance(formatted, str)
...
class TestColorConsistency:
    ...
    def test_all_methods_return_colored_output(self):
        ...
        # Progress bar
        assert ColorStripper.has_colors(ModernVisualEngine.create_progress_bar(50, 100, tool="test"))

        # Vulnerability card
        assert ColorStripper.has_colors(
            ModernVisualEngine.format_vulnerability_card({"severity": "high", "name": "Test", "description": "Test"})
        )

        # Error card
        assert ColorStripper.has_colors(ModernVisualEngine.format_error_card("ERROR", "tool", "message"))

        # Tool status
        assert ColorStripper.has_colors(ModernVisualEngine.format_tool_status("tool", "RUNNING", "target"))
