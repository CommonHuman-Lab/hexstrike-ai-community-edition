...
export const DEMO_TOOLS: Tool[] = [
  ...
  { name: 'qsreplace',    desc: 'Replace query-string parameter values in a list of URLs',  category: 'web_recon', endpoint: '/api/tools/qsreplace',    method: 'POST', params: { urls: r() },   optional: { replacement: 'FUZZ', additional_args: '' },                                                                                                      effectiveness: 0.72 },
  ...
  { name: 'burpsuite',           desc: 'Burp Suite web application security testing platform',                   category: 'web_vuln', endpoint: '/api/tools/burpsuite-alternative',method: 'POST', params: { target: r() },        optional: { scan_type: 'comprehensive', headless: true, max_depth: 3, max_pages: 50 },        effectiveness: 0.90 },
  ...
  { name: 'curl',                desc: 'HTTP/S request tool — manual probing and PoC verification',              category: 'web_vuln', endpoint: '/api/tools/http-framework',       method: 'POST', params: { url: r() },           optional: { method: 'GET', data: '', headers: '' },                                            effectiveness: 0.75 },
  { name: 'httpie',              desc: 'Human-friendly HTTP client for API testing and inspection',               category: 'web_vuln', endpoint: '/api/tools/http-framework',       method: 'POST', params: { url: r() },           optional: { method: 'GET', data: '', headers: '' },                                            effectiveness: 0.75 },
  ...
  { name: 'anew',           desc: 'Append unique lines — deduplication for recon pipelines',                 category: 'osint', endpoint: '/api/tools/anew',           method: 'POST', params: { input_data: r() }, optional: { output_file: '', additional_args: '' },               effectiveness: 0.70 },
  { name: 'uro',            desc: 'Filter and deduplicate URL lists — remove noise from recon output',       category: 'osint', endpoint: '/api/tools/uro',            method: 'POST', params: { urls: r() },      optional: { whitelist: '', blacklist: '', additional_args: '' },    effectiveness: 0.75 },
  ...
  { name: 'shodan-cli',     desc: 'Shodan CLI — search internet-connected devices and services',             category: 'osint', endpoint: '/api/tools/shodan',         method: 'POST', params: { query: r() },    optional: { additional_args: '' },                                   effectiveness: 0.88 },
  { name: 'censys-cli',     desc: 'Censys CLI — internet asset discovery and certificate analysis',          category: 'osint', endpoint: '/api/tools/censys',         method: 'POST', params: { query: r() },    optional: { additional_args: '' },                                   effectiveness: 0.85 },
  { name: 'have-i-been-pwned', desc: 'Check if credentials or domains appear in breach data',               category: 'osint', endpoint: '/api/tools/hibp',           method: 'POST', params: { query: r() },    optional: { additional_args: '' },                                   effectiveness: 0.80 },
  ...
  { name: 'bulk-extractor', desc: 'Extract features (emails, URLs, credit cards) from disk images',          category: 'forensics', endpoint: '/api/tools/bulk_extractor', method: 'POST', params: { input_file: r() },             optional: { output_dir: '/tmp/bulk_extractor_output', scanners: '', additional_args: '' },                  effectiveness: 0.82 },
...
const HEALTH_CAT_TOOLS: Record<string, string[]> = {
  ...
  network:       ['rustscan', 'masscan', 'autorecon', 'nbtscan', 'arp-scan', 'responder', 'nxc', 'enum4linux-ng', 'rpcclient', 'enum4linux'],
  web_security:  ['ffuf', 'feroxbuster', 'dirsearch', 'dotdotpwn', 'xsser', 'wfuzz', 'gau', 'waybackurls', 'arjun', 'paramspider', 'x8', 'jaeles', 'dalfox', 'httpx', 'wafw00f', 'burpsuite', 'zaproxy', 'katana', 'hakrawler'],
  ...
  forensics:     ['vol', 'steghide', 'hashpump', 'foremost', 'exiftool', 'strings', 'xxd', 'file', 'photorec', 'testdisk', 'scalpel', 'bulk-extractor', 'stegsolve', 'zsteg', 'outguess'],
  ...
  osint:         ['amass', 'subfinder', 'fierce', 'dnsenum', 'theharvester', 'sherlock', 'social-analyzer', 'recon-ng', 'maltego', 'spiderfoot', 'shodan-cli', 'censys-cli', 'have-i-been-pwned', 'whois', 'bbot'],
  exploitation:  ['msfconsole', 'msfvenom', 'searchsploit'],
  api:           ['api-schema-analyzer', 'postman', 'insomnia', 'curl', 'httpie', 'anew', 'qsreplace', 'uro'],
  wireless:      ['kismet', 'wireshark', 'tshark', 'tcpdump', 'airbase-ng', 'airdecap-ng', 'hcxdumptool', 'hcxpcapngtool', 'mdk4', 'eaphammer', 'wifite', 'bettercap'],
  additional:    ['smbmap', 'volatility', 'sleuthkit', 'autopsy', 'evil-winrm', 'airmon-ng', 'airodump-ng', 'aireplay-ng', 'aircrack-ng'],
