#!/usr/bin/env python3
"""
HTTP Testing Framework - Burp Suite Alternative for HexStrike

This module provides advanced HTTP testing capabilities including:
- Request interception and modification
- Match & replace rules
- Scope management
- Intruder-style fuzzing (Sniper mode)
- Website spidering
- Vulnerability analysis
- Proxy history tracking

Acts as a lightweight alternative to Burp Suite for automated testing scenarios.
"""
...
class HTTPTestingFramework:
    ...
    def setup_proxy(self, proxy_port: int = 8080):
        ...
        self.session.proxies = {"http": f"http://127.0.0.1:{proxy_port}", "https": f"http://127.0.0.1:{proxy_port}"}

    def intercept_request(
        self, url: str, method: str = "GET", data: dict = None, headers: dict = None, cookies: dict = None
    ) -> dict:
        ...
        try:
            ...
            logger.error(f"HTTP Framework Error: {str(e)}")
    ...
    def _apply_match_replace(self, url: str, data, headers: dict):
        ...
        for rule in self.match_replace_rules:
            ...
            try:
                if where == "url":
                    ...
                    out_headers = {
                        re.sub(pattern, repl, k): re.sub(pattern, repl, str(v)) for k, v in out_headers.items()
                    }
                elif where == "body":
                    if isinstance(out_data, dict):
                        out_data = {
                            re.sub(pattern, repl, k): re.sub(pattern, repl, str(v)) for k, v in out_data.items()
                        }
        ...
        if not self._in_scope(url):
            logger.warning(f"HTTP-Framework: Out of scope: {url}")
        ...
        for p in params:
            for pay in payloads:
                ...
                if not resp.get("success"):
                    continue
                r = resp["response"]
                changed = (base_status is not None and r.get("status_code") != base_status) or (
                    base_len is not None and abs(r.get("size", 0) - base_len) > 150
                )
