...
def test_error_recovery():
    ...
    try:
        ...
        context = {"target": target, "parameters": data.get("parameters", {}), "attempt_count": 1}
...
def execute_with_recovery_endpoint():
    ...
    try:
        ...
        result = execute_command_with_recovery(
            tool_name=tool_name, command=command, parameters=parameters, use_cache=use_cache, max_attempts=max_attempts
        )

        return jsonify(
            {"success": result.get("success", False), "result": result, "timestamp": datetime.now().isoformat()}
        )
