    ...
    def parse_output(self, stdout: str, stderr: str, returncode: int) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if cipher_match:
                ...
                cipher_info = {"protocol": protocol, "cipher": cipher, "bits": int(bits), "status": status}
                ...
                if int(bits) < 128 or any(
                    weak in cipher.upper() for weak in ["DES", "RC4", "MD5", "NULL", "EXPORT", "anon"]
                ):
            ...
            if "Heartbleed" in line:
                if "vulnerable" in line.lower():
                    vulnerabilities.append({"name": "Heartbleed", "status": "VULNERABLE", "severity": "CRITICAL"})
            ...
            if "renegotiation" in line.lower():
                if "insecure" in line.lower() or "vulnerable" in line.lower():
                    vulnerabilities.append(
                        {"name": "Insecure Renegotiation", "status": "VULNERABLE", "severity": "MEDIUM"}
                    )
            ...
            if "compression" in line.lower():
                if "enabled" in line.lower():
                    vulnerabilities.append(
                        {"name": "TLS Compression (CRIME)", "status": "ENABLED", "severity": "MEDIUM"}
                    )
